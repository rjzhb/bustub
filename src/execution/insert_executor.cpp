...
InsertExecutor::InsertExecutor(ExecutorContext *exec_ctx, const InsertPlanNode *plan,
  ...
  plan_ = plan;
  exec_ctx_ = exec_ctx;
  tableInfo = exec_ctx_->GetCatalog()->GetTable(plan_->TableOid());
  index_array_ = exec_ctx->GetCatalog()->GetTableIndexes(tableInfo->name_);
}
...
auto InsertExecutor::Next([[maybe_unused]] Tuple *tuple, RID *rid) -> bool {
  ...
  if (plan_->IsRawInsert()) {
    if (next_insert_ != raw_vals.size()) {
      *tuple = Tuple(raw_vals[next_insert_++], &tableInfo->schema_);
      is_insert = tableInfo->table_->InsertTuple(*tuple, rid, exec_ctx_->GetTransaction());
    }
  ...
  if (is_insert && !index_array_.empty()) {
    for (auto &index_info : index_array_) {
      const auto key_tuple =
          tuple->KeyFromTuple(tableInfo->schema_, index_info->key_schema_, index_info->index_->GetKeyAttrs());
